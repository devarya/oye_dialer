 ...
 - (void)loadView
{
  ...
  [menuView setTitle:NSLocalizedString(@"keypad", @"Call View")
                image:[UIImage imageNamed:@"keypaddialer.png"] forPosition:1];
...
- (void)viewWillAppear:(BOOL)animated
{
  ...
  _new_call = PJSUA_INVALID_ID;
    ...
    if ([self getSystemVersion] >= 4.0) {
        [audioSession setActive:NO withFlags:kAudioSessionSetActiveFlag_NotifyOthersOnDeactivation error: nil];
    }
    else {
...
{
  [[SipManager voipManager] callDisconnecting];
  
  dtmfCmd = nil;
  [self setSpeakerPhoneEnabled:YES];
  [self setMute:NO];
  ...
  if (pool)
  {
    ...
    if (url != NULL)
    {
      ...
      ABRecordRef record = [self findRecord:[NSString stringWithUTF8String:
                                             pj_strbuf(&dst)]];
      if (record)
        phoneNumber = (__bridge NSString *)ABRecordCopyCompositeName(record);
      if (!phoneNumber)
      {
        if (url->display.slen)
        {
...
- (void)processCall:(NSDictionary *)userInfo
{
  ...
  state = [[userInfo objectForKey: @"State"] intValue];
    if (state == 4) {
        //[callViewController showDialPad:YES];
        [self enableMenuItems:YES isCalling:NO];
    }
  switch(state)
  {
    case PJSIP_INV_STATE_CALLING: // After INVITE is sent.
    [self enableMenuItems:NO isCalling:YES];
    ...
    case PJSIP_INV_STATE_INCOMING: // After INVITE is received.
      ...
      {
        [self enableMenuItems:NO isCalling:NO];
        [self.view addSubview: _dualBottomBar];
        [self showKeypad:NO animated:NO];
        [self.view addSubview:_containerView];
        [_lcd setText:@""];
...
- (float)getSystemVersion
{
    ...
    BOOL needsVersionCheck = YES;
    if (needsVersionCheck) {
        NSString *version = [[UIDevice currentDevice] systemVersion];
        returnValue = [version floatValue];
        
        needsVersionCheck = NO;
    }
...
-(void)showDialPad:(BOOL)flag
{
    MenuCallView *menuView = (MenuCallView *)_switchViews[1];
    [menuView showKeyPad:flag];
}

-(void)enableMenuItems:(BOOL)flag isCalling:(BOOL)isCalling
{
    MenuCallView *menuView = (MenuCallView *)_switchViews[1];
    [menuView enableMenuItems:flag isCalling:isCalling];
}
