...
//  NSNumber *value;
  ...
  if (ci.state == PJSIP_INV_STATE_DISCONNECTED) // Session is terminated.
  {
    ...
    sip_ring_stop([[SipManager voipManager] pjsipConfig]);
  ...
  else if (ci.state == PJSIP_INV_STATE_EARLY) 
  {
        ...
        ci.media_status == PJSUA_CALL_MEDIA_NONE) 
    {
      ...
      sip_ringback_start([[SipManager voipManager] pjsipConfig]);
    }
	}
           ...
           pjsip_rx_data *rdata)
{
  ...
  /* FIXME: Start ringback */
  sip_ring_start([[SipManager voipManager] pjsipConfig]);
...
pj_status_t sip_startup(app_config_t *app_config)
{
  ...
  if (val < 0 || val > 65535)
  {
    ...
    [[SipManager voipManager] displayParameterError:
...
pj_status_t sip_connect(pj_pool_t *pool, pjsua_acc_id *acc_id)
{
  ...
  uname  = [[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_USERID] mutableCopy] UTF8String];
  authname  = [[[NSUserDefaults standardUserDefaults] stringForKey: 
                @"authname"] UTF8String];
  contactname  = [[[NSUserDefaults standardUserDefaults] stringForKey: 
                @"contact"] UTF8String];
  passwd = [[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_PWD] mutableCopy] UTF8String];
  server = [[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_PROXY] mutableCopy] UTF8String];
  ...
  if ((status = pjsua_verify_sip_url(acc_cfg.id.ptr)) != 0) 
  {
    PJ_LOG(1,(THIS_FILE, "Error: invalid SIP URL '%s' in local id argument", 
      acc_cfg.id));
    [[SipManager voipManager] displayParameterError: @"Invalid value for username or server."];
    return status;
  }
  ...
  if ((status = pjsua_verify_sip_url(acc_cfg.reg_uri.ptr)) != 0) 
  {
    PJ_LOG(1,(THIS_FILE,  "Error: invalid SIP URL '%s' in registrar argument",
      acc_cfg.reg_uri));
    [[SipManager voipManager] displayParameterError: @"Invalid value for server parameter."];
    return status;
  }
  ...
  // FIXME: gestion du message 423 dans pjsip
    acc_cfg.reg_timeout = [[[NSUserDefaults standardUserDefaults] valueForKey:SIP_REGISTRATIONTIMEOUT] intValue]>0?[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_REGISTRATIONTIMEOUT] intValue]:1200;
  ...
  while (anObject = [enumerator nextObject])
  {
    ...
    if (proxy && strlen(proxy))
    {
      ...
      if ((status = pjsua_verify_sip_url(pj_proxy.ptr)) != 0) 
      {
        ...
        [mutableStr release];
        [[SipManager voipManager] displayParameterError: @"Invalid value for proxy parameter."];
        continue;
      }
 ...
 *acc_id = aid;
  ...
  if (status != PJ_SUCCESS) 
  {
    ...
    [[SipManager voipManager] displayParameterError: @"Error adding new account."];
  }
#endif  
  return status;
}
  ...
  pjsua_call_id *call_id)
{
  ...
  sip_domain = [[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_PROXY] mutableCopy] UTF8String];
...
pj_status_t sip_add_account(NSDictionary *account, pjsua_acc_id *acc_id)
{
  ...
  uname  = [USER_NAME UTF8String];
  passwd = [PASSWORD UTF8String];
  server = [SERVER UTF8String];
  ...
  // FIXME: gestion du message 423 dans pjsip
  acc_cfg.reg_timeout = [[[NSUserDefaults standardUserDefaults] valueForKey:SIP_REGISTRATIONTIMEOUT] intValue]>0?[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_REGISTRATIONTIMEOUT] intValue]:1200;
    NSLog(@"%d",[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_REGISTRATIONTIMEOUT] intValue]>0?[[[NSUserDefaults standardUserDefaults] valueForKey:SIP_REGISTRATIONTIMEOUT] intValue]:1200);
  // FIXME : verify in settings view
  if (acc_cfg.reg_timeout < 1 || acc_cfg.reg_timeout > 3600) 
  {
    PJ_LOG(1,(THIS_FILE, 
